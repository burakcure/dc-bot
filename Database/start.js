...
const oldFileSync=async (db)=>{
  ...
  keys.forEach(async (element) =>{
    await db.models.customCommands.create({GuildID:element.substring(0,18),OwnerID:element.substring(element.length-18,element.length),CommandName:element.substring(18,element.length-18),CommandType:commandList[element].Type,CommandLink:commandList[element].Link})
