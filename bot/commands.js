...
const getCommandFromMessage = (message) => {
  if (
    cooldownOfUsers[message.author.tag] === undefined ||
    cooldownOfUsers[message.author.tag] == 0
  ) {
  ...
  } else message.reply("You can only send command every 3 seconds");

  return -1;
};

const isCustomCommand = async (commandText, userId, guildId, db) => {
  let getFromDB = await db.findOne({
    where: { GuildID: guildId, OwnerID: userId, CommandName: commandText },
  });
  if (getFromDB !== null) {
    return getFromDB;
  }
...
module.exports = {
  ...
  commandProcess: async (message, client, db) => {
    ...
    if (getCommand != -1)
      try {
        let checkCustom = await isCustomCommand(
          getCommand,
          userId,
          guildId,
          db
        ...
        let type = 0;
        if (checkCustom == false) type = commandList[getCommand];
        else type = checkCustom.CommandType;
        ...
        switch (parseInt(type)) {
          ...
          default:
            message.reply(
              `There is no command named **${getCommand}** you can look at the list of commands using .help`
            );
        }
      } catch {
